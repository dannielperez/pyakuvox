# pyakuvox — Field Learnings (validated 2026-06-23, ~60-device fleet)

Hard-won facts from configuring Akuvox intercoms (R29/R29C/X916/S535/R27) across
~60 sites over WireGuard VPN. Drove the fixes below; the rest are TODOs.
...
- Old firmware negotiates weak DH / legacy TLS → modern OpenSSL refuses. **`legacy_tls`
  setting added** (permissive SSL ctx: min-version lowered + `SECLEVEL=0`). Needed on ~7 units.
...
- Fleet was mixed: some digest(4) (usable), many whitelist(2) (locked out).
...
period — was **1800s** fleet-wide, the real cause of slow failover; set to **30**).
...
- **SPA-firmware web-UI client (HIGH):** newer firmware replaced the `/fcgi/do` nonce flow with a
  modern SPA — `WebUIClient.login()` fails with "Failed to get encryption nonce". ~21 of 60 devices
  are unreachable for auth-mode flips until this is built (reverse the SPA login + config endpoints).
