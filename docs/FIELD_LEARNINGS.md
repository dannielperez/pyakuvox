...
## X916 intercom configuration surface (916.30.10.357 verified)
- Intercom theme / Homepage: `Config.DoorSetting.GENERAL.Theme=2` and
  `BuildingDisplayType=0`.
- Homepage key types: `5=Speed Dial`, `2=PIN`, `6=Null`; this model exposes six
  homepage slots.
- Relay control: `DTMF.Enable=1`, `DTMF.Option=0` is one-digit DTMF, and
  `DTMF.DtmfWhitelist=2` is **All Numbers**.
- Relays A-D support monostable/default-state operation, trigger delay, hold
  delay, and caller-selected one-digit codes.
- Use `AkuvoxDevice.ensure_intercom_configuration(..., apply=True)` with an
  explicit caller-owned configuration. It refuses to guess fields, verifies the
  full readback after one write, and returns an `unsupported-*` non-mutating
  outcome when the model, requested shape, or firmware surface is unavailable.
